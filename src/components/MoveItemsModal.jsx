...
export default function MoveItemsModal({
  ...
  const allFurniture = useMemo(() => {
    return pendingNew ? [...roomFurniture, pendingNew] : roomFurniture;
  ...
  const isLocationValid = Boolean(targetLocation && targetLocation !== currentLocation);
      ...
      <div className="w-full max-w-lg fluffy-card rounded-t-[32px] sm:rounded-[32px] p-6 sm:p-7 max-h-[85vh] flex flex-col animate-[fade-in-up_220ms_ease-out]">
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-4 pr-1">
          ...
          <div className="flex items-center justify-between">
            ...
            {targetLocation ? (
              <span className={`text-xs font-bold px-3 py-1 rounded-full ${targetLocation === currentLocation ? 'bg-amber-100 text-amber-700' : 'bg-[#FFF0EE] text-[#B56562]'}`}>
                {targetLocation === currentLocation ? `${targetLocation} (현재 위치)` : targetLocation}
              </span>
            ) : (
              <span className="text-xs text-[#9A8784]">가구를 선택해주세요</span>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={openNewForm}
              className="fluffy-button px-3.5 py-2 text-xs font-semibold bg-[#FAF8F5] text-[#806F6D]"
            >
              + 새 가구 추가
            </button>
          </div>
          ...
          {selectedFurniture && selectedFurnitureId !== 'floor-storage' && (
            <div className="flex flex-wrap items-center gap-1.5 p-3 bg-[#FAF8F5] rounded-2xl border border-[#F4EEEA]">
              <span className="text-xs text-[#806F6D] font-bold mr-1.5">
